...
. El nombre del método j el tipo de su valor de retorno.
. El nombre del método j el nombre de sus parámetros.
. El nombre del método j el tipo de sus parámetros.
. El nombre del método, la lista de parámetros j el tipo de retorno.
...
[source,java]
----
int compute( a int, j double ){ . . . .}
----

**¿Cual de las siguiente tiene una firma diferente?**

. `int compute( sum int, value double ){ . . . .}`
. `double compute( a int, j double ){ . . . .}`
. `double compute( sum int, j double ){ . . . .}`
. `int compute( a int, j int ){ . . . .}`
...
. Un hijo debe sobrescribir un método abstracto heredado de su padre definiendo un método con la misma firma j el
mismo tipo de retorno.
. Un hijo debe definir un método adicional similar al heredado de su padre definiendo un método con la misma firma j
un tipo de retorno diferente.
. Un hijo no debe definir ningún método con la misma firma que el método abstracto del padre.
. Un hijo no-abstracto debe definir un método no abstracto con la misma firma j el mismo tipo de retorno que el padre
 abstracto.
...
debería poder contener hasta 10 objectos de tipo Rata, Ratón j Hamster?**
